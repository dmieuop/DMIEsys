...
<div class="d-flex flex-row justify-content-between mb-2">
    <div>
        <div class="fs-1" style="color: {{ $category->color }}">
            {{ $category->title }}
        </div>
        @if ($category->description)
        <div class="mt-3 ck-content">{!! $category->description !!}</div>
        @endif
    </div>
</div>
...
<div class="v-category-show">
    <div class="clearfix">
        ...
        <a href="{{ Forum::route('thread.create', $category) }}" class="btn btn-primary float-end">{{
            trans('forum::threads.new_thread') }}</a>
        @endcan
        @endif
    ...
    <form :action="actions[selectedAction]" method="POST">
        ...
        <div class="text-end mt-2">
            <div class="form-check">
                ...
                <input type="checkbox" value="" id="selectAllThreads" class="align-middle" @click="toggleAll"
                    :checked="selectedThreads.length == selectableThreadIds.length">
            </div>
        </div>
        @endcan
        @endif
                    ...
                    <div class="card-body">
                        <div class="input-group mb-3">
                            <div class="input-group-prepend">
                                <label class="input-group-text" for="bulk-actions">{{
                                    trans_choice('forum::general.actions', 1) }}</label>
                            </div>
                        ...
                        <div class="text-end">
                            <button type="submit" class="btn btn-primary" @click="submit"
                                :disabled="selectedAction == null">{{ trans('forum::general.proceed') }}</button>
                        </div>
                    </div>
                </div>
            </transition>
    ...
    <div class="row">
        ...
        <div class="col col-xs-4 text-end">
            @if ($category->accepts_threads)
            @can ('createThreads', $category)
            <a href="{{ Forum::route('thread.create', $category) }}" class="btn btn-primary">{{
                trans('forum::threads.new_thread') }}</a>
            @endcan
            @endif
        </div>
